...
void cpu() {
    ...
    while (1) {
        ...
        if ((t % b) == 0 && a) {
            ...
            //                printf("(I/O REQUEST) Process %s has been
            //                scheduled.\n",
        ...
        if (!kernel->scheduler->scheduled_proc) {
            ...
            if (kernel->scheduler->scheduled_proc)
                ...
                printf("No process has been scheduled (2).\n");
            continue;
        }
...
int main() {
    kernel_init();
    segment_table_t* seg_table = kernel->seg_table;
    segment_t* new_seg = init_segment(2, 32, 1, 900);
    add_segment(seg_table, new_seg);

    segment_t* same_seg = get_segment(seg_table, 2);
    segment_t* null_seg = get_segment(seg_table, 1);
    printf("%d\n", same_seg->id);
    !null_seg ? printf("Is null\n") : printf("%d\n", null_seg->id);
}
